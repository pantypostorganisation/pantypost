...
const PropsSchema = z.object({
  ...
  codePhoto: z.string().optional(),
  idFront: z.string().optional(),
  idBack: z.string().optional(),
  passport: z.string().optional(),
  getTimeAgo: z.function().args(z.any()).returns(z.string()).optional(),
});

function formatDateRelative(getTimeAgo?: (v: unknown) => string, when?: unknown): string {
  try {
    if (getTimeAgo && when) {
      const txt = getTimeAgo(when);
      return sanitizeStrict(txt || 'recently');
    }
  } catch {
    /* ignore */
  }
...
export default function PendingState(rawProps: PendingStateProps) {
  const parsed = PropsSchema.safeParse(rawProps);
  const { user, code, codePhoto, idFront, idBack, passport, getTimeAgo } = parsed.success
    ? parsed.data
    : { user: {}, code: '', getTimeAgo: undefined };
  ...
  const requestedAgo = formatDateRelative(getTimeAgo, (user as any)?.verificationRequestedAt);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black to-[#0a0a0a] text-white py-10 px-4 sm:px-6">
        ...
        <div className="bg-[#121212] rounded-xl shadow-xl overflow-hidden border border-yellow-800">
          ...
          <div className="p-6 sm:p-8">
            <div className="flex flex-col items-center justify-center text-center mb-8">
              ...
              <p className="text-sm text-gray-400 mt-2">Requested {requestedAgo}</p>

              <div className="mt-8 border-t border-gray-800 pt-6 w-full">
                <div className="flex justify-between items-center text-sm text-gray-400 mb-3">
                  <span>Verification Status:</span>
                  <span className="px-3 py-1 bg-yellow-900 text-yellow-400 rounded-full text-xs font-medium">Pending Review</span>
                </div>

                <div className="flex justify-between items-center text-sm text-gray-400 mb-3">
                  <span>Verification Code:</span>
                  <span className="font-mono bg-black px-2 py-1 rounded text-yellow-500">{safeCode || '—'}</span>
                </div>
            ...
            <div className="rounded-lg bg-[#1a1a1a] border border-gray-800 p-4 mt-6">
              ...
              <p className="text-sm text-gray-300">
                Our admin team will review your documents and either approve or reject your verification request. You'll receive a
      ...
      <ImageViewerModal imageData={currentImage} onClose={() => setCurrentImage(null)} />
    </div>
  );
}
