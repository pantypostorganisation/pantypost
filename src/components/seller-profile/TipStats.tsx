...
import { tipService, TipStats as TipStatsType } from '@/services/tip.service';
...
const RecentTipSchema = z.object({
  from: z.string().default('Anonymous'),
  amount: z.number().nonnegative().catch(0),
});

const TipStatsSchema = z.object({
  totalAmount: z.number().nonnegative().catch(0),
  averageTip: z.number().nonnegative().catch(0),
  totalTips: z.number().int().nonnegative().catch(0),
  uniqueTippers: z.number().int().nonnegative().catch(0),
  recentTips: z.array(RecentTipSchema).default([]),
});

export default function TipStatsComponent({ username, isOwnProfile = false }: TipStatsProps) {
  const [stats, setStats] = useState<TipStatsType | null>(null);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        const parsed = TipStatsSchema.safeParse(data);
        setStats(parsed.success ? (parsed.data as unknown as TipStatsType) : null);
      } catch (error) {
  ...
  const totalAmountLabel = formatCurrency(stats.totalAmount);
    ...
    <div className="bg-gray-800/50 backdrop-blur border border-gray-700 rounded-lg p-4">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-gray-900/50 rounded p-3">
          ...
          <div className="text-2xl font-bold text-green-400">{totalAmountLabel}</div>
        </div>
        ...
        <div className="bg-gray-900/50 rounded p-3">
          ...
          <div className="text-2xl font-bold text-white">{averageTipLabel}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-gray-900/50 rounded p-3">
          ...
          <div className="text-xl font-semibold text-white">{stats.totalTips}</div>
        </div>
        ...
        <div className="bg-gray-900/50 rounded p-3">
          ...
          <div className="text-xl font-semibold text-white">{stats.uniqueTippers}</div>
        </div>
      </div>
