...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  // FIXED: Update bidder tracking when a bid is placed
  const updateBidderTracking = useCallback(async (
    ...
    // FIXED: For incremental bids, we should track the CURRENT total paid, not accumulate
    tracking[listingId][bidder] = {
      bidder,
      totalBidAmount: bidAmount,
      totalPaidAmount: hasPendingOrder 
        ? paidAmount // For new bids, this is the full amount
        : (existingRecord ? existingRecord.totalPaidAmount + paidAmount : paidAmount), // For incremental, add to existing
      hasPendingOrder,
  ...
  // FIXED: Handle incremental bid charge with tracking
  const chargeIncrementalBid = useCallback(async (
    ...
    try {
      ...
      return await transactionLock.current.acquireLock(`incremental_bid_${listingId}_${validatedBidder}`, async () => {
        ...
        // Update bidder tracking with the incremental amount
  ...
  // FIXED: NEW placeBid function with proper initial minimum bid validation
  const placeBid = useCallback(async (listingId: string, bidder: string, amount: number): Promise<boolean> => {
    try {
      ...
      // Get the current listing to check auction details
      // NOTE: You'll need to implement a way to get the current listing
      // This is a placeholder - you should replace this with your actual listing service call
      let listing: any = null;
      
      // PLACEHOLDER: Replace this with your actual listing service call
      // Example: const listingResult = await listingsService.getListing(validatedListingId);
      // if (!listingResult.success || !listingResult.data) {
      //   console.error('[PlaceBid] Could not find listing');
      //   return false;
      // }
      // listing = listingResult.data;

      // For now, we'll work with the information we have
      // You should implement proper listing retrieval here
      ...
      const currentPendingOrder = orderHistory.find(order => 
      ...
      const existingBids = orderHistory.filter(order => 
        order.listingId === validatedListingId && 
        order.shippingStatus === 'pending-auction'
      );

      // Find the current highest bid amount
      let currentHighestBid = 0;
      if (existingBids.length > 0) {
        currentHighestBid = Math.max(...existingBids.map(order => order.price));
      }

      // Get starting price from listing or use a default
      // PLACEHOLDER: Replace with actual starting price from listing
      const startingPrice = listing?.auction?.startingPrice || 0;

      console.log('[PlaceBid] Bid validation data:', {
        currentHighestBid,
        startingPrice,
        validatedAmount,
        existingBidsCount: existingBids.length
      });

      // FIXED: Proper bid validation
      let isValidBid = false;
      let validationMessage = '';

      if (currentHighestBid === 0) {
        // First bid - must be at least starting price
        if (validatedAmount >= startingPrice) {
          isValidBid = true;
          validationMessage = 'Valid first bid';
        } else {
          validationMessage = `First bid must be at least ${startingPrice.toFixed(2)}`;
        }
      } else {
        // Subsequent bid - must be higher than current highest
        if (validatedAmount > currentHighestBid) {
          isValidBid = true;
          validationMessage = 'Valid higher bid';
        } else {
          validationMessage = `Bid must be higher than current highest bid of ${currentHighestBid.toFixed(2)}`;
        }
      }

      console.log('[PlaceBid] Validation result:', {
        isValidBid,
        validationMessage
      });

      if (!isValidBid) {
        console.error('[PlaceBid] Invalid bid:', validationMessage);
        return false;
      }

      // Use transaction lock to prevent race conditions
      return await transactionLock.current.acquireLock(`bid_${validatedListingId}_${validatedBidder}`, async () => {
        try {
          // If user already has a pending bid, refund it first
          if (currentPendingOrder) {
            console.log('[PlaceBid] Refunding previous bid before placing new one');
          ...
          // For incremental bidding by the same user, charge the difference
          let chargeAmount = totalWithFee;
          if (currentPendingOrder && currentPendingOrder.buyer === validatedBidder) {
            const previousTotal = currentPendingOrder.markedUpPrice;
            chargeAmount = totalWithFee - previousTotal;
            
            console.log('[PlaceBid] Incremental bid detected:', {
              previousTotal,
              newTotal: totalWithFee,
              chargeAmount
            });

            // Check if user has enough for the additional amount
            if (chargeAmount > 0 && buyerCurrentBalance < chargeAmount) {
          ...
          if (chargeAmount > 0) {
            await setBuyerBalance(validatedBidder, buyerCurrentBalance - chargeAmount);
          }

          // Hold the bid funds using existing function
          const holdSuccess = await holdBidFunds(
            validatedListingId,
            validatedBidder,
            validatedAmount,
            listing?.title || `Auction ${validatedListingId}`
          );

          if (!holdSuccess) {
            // Rollback balance change if hold failed
            if (chargeAmount > 0) {
              await setBuyerBalance(validatedBidder, buyerCurrentBalance);
            }
  ...
  }, [getBuyerBalance, setBuyerBalance, orderHistory, holdBidFunds, refundBidFunds]);
