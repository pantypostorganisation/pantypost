...
const { deleteFile, getFileUrl } = require('../config/upload.config');
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        role: user.role,
        // IMPORTANT: Include gallery images for sellers
...
// GET /api/users/:username/gallery - Get user gallery
router.get('/:username/gallery', async (req, res) => {
  try {
    const user = await User.findOne({ username: req.params.username })
      .select('galleryImages role username');
    ...
    // Only sellers have galleries
    if (user.role !== 'seller') {
      return res.status(400).json({
        success: false,
        error: {
          code: ERROR_CODES.ACTION_NOT_ALLOWED,
          message: 'Only sellers have galleries'
        }
      });
    }
...
// PATCH /api/users/:username/gallery - Update gallery (add/remove images)
router.patch('/:username/gallery', authMiddleware, async (req, res) => {
  try {
    // Only user themselves can update their gallery
    if (req.user.username !== req.params.username && req.user.role !== 'admin') {
      return res.status(403).json({
        success: false,
        error: {
    ...
    // Only sellers can have galleries
    if (user.role !== 'seller') {
      return res.status(400).json({
        success: false,
        error: {
          code: ERROR_CODES.ACTION_NOT_ALLOWED,
          message: 'Only sellers can have galleries'
        }
      });
    }
    
    const { action, images, imageUrl, imageIndex } = req.body;
    ...
    if (!action) {
      return res.status(400).json({
        ...
        error: {
          ...
          message: 'Action is required (add, remove, replace, clear)'
        }
      });
    }
    ...
    let updatedGallery = [...user.galleryImages];
    ...
    switch (action) {
        ...
        if (!images || !Array.isArray(images)) {
          return res.status(400).json({
            ...
            error: {
              ...
              message: 'Images array is required for add action'
            }
          });
        }
        
        // Add new images (up to 20 total)
        updatedGallery = [...updatedGallery, ...images].slice(0, 20);
        break;
      ...
      case 'remove':
        if (imageIndex === undefined && !imageUrl) {
          return res.status(400).json({
            ...
            error: {
              code: ERROR_CODES.INVALID_INPUT,
              message: 'Either imageIndex or imageUrl is required for remove action'
            }
          });
        }
        ...
        if (imageIndex !== undefined) {
          // Remove by index
          if (imageIndex >= 0 && imageIndex < updatedGallery.length) {
            updatedGallery.splice(imageIndex, 1);
          }
        } else if (imageUrl) {
          // Remove by URL
          updatedGallery = updatedGallery.filter(img => img !== imageUrl);
        }
        break;
      ...
      case 'replace':
        if (!images || !Array.isArray(images)) {
          return res.status(400).json({
            ...
            error: {
              code: ERROR_CODES.INVALID_INPUT,
              message: 'Images array is required for replace action'
            }
          });
        }
        
        // Replace entire gallery
        updatedGallery = images.slice(0, 20);
        break;
      ...
      default:
        return res.status(400).json({
          ...
          error: {
            code: ERROR_CODES.INVALID_INPUT,
            message: 'Invalid action. Must be add, remove, replace, or clear'
          }
        });
    }
    ...
    user.markModified('galleryImages'); // CRITICAL: Ensure Mongoose knows the array changed
    await user.save();
    
    // Log for debugging
    console.log(`Gallery updated for user ${user.username}:`, {
      action,
      previousCount: (req.body.action === 'replace' ? 0 : user.galleryImages.length),
      newCount: updatedGallery.length,
      images: updatedGallery.slice(0, 3) // Log first 3 URLs for debugging
    });
